...
[Core Keeper](https://corekeeper.net/) 공략 도감 웹앱. 상단 탭으로 5개 도감을 전환합니다.
...
- **필터 / 정렬** — 하위분류(투구/상의/하의, 반지/목걸이/오프핸드, 곡괭이/삽/… ) · 레벨·이름순 · 이름/효과/재료 검색
- **아이콘** — 픽셀아트 스프라이트 (위키 미문서화 항목 제외, 없으면 자리표시)

> 데이터는 위키 중앙 데이터 모듈에서 [`scrape-weapons.mjs`](scrape-weapons.mjs)(무기) · [`scrape-gear.mjs`](scrape-gear.mjs)(방어구/장신구/아이템)로 재생성합니다.
...
scrape-lib.mjs        # 스크래핑 공용 유틸 (Lua 파서 · fetch · 아이콘 다운로드)
...
public/
  ...
  gear-icons/         # 방어구/장신구/아이템 아이콘 + manifest.json
  data/               # weapons.json · armor.json · accessories.json · tools.json
```
