...
[Core Keeper](https://corekeeper.net/) 공략 도감 웹앱. 상단 탭으로 9개 도감을 전환합니다.
...
- **🐾 펫** — **펫 14종**: 유형(근접/원거리/버프형) + 따라다니며 주는 패시브 버프 + 특화
...
### 🐾 펫 도감
- **펫 14종** — 유형(근접/원거리/버프형) · **따라다니며 주는 패시브 버프** · 특화 가능 스탯 · 등장 지역
- **필터 / 검색** — 유형 칩 · 펫명·효과 검색
...
scrape-pets.mjs       # 🐾 펫 스크래퍼
build-recos.mjs       # 🏆 추천 빌드(큐레이션) → builds.json, 도감 아이템에 연결
public/
  icons/              # 재료 아이콘(webp) + manifest.json
  weapon-icons/ · gear-icons/ · fish-icons/ · boss-icons/ · pet-icons/   # 각 도감 아이콘 + manifest.json
  data/               # weapons · armor · accessories · tools · fish · bosses · pets · builds (.json)
```
...
node scrape-pets.mjs      # 펫 → public/data/pets.json + public/pet-icons/
```
