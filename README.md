...
[Core Keeper](https://corekeeper.net/) 공략 도감 웹앱. 상단 탭으로 6개 도감을 전환합니다.
...
- **🎣 낚시** — **낚시터 10곳 · 물고기 44종**: 어종별 낚일 확률 + 필요 낚시 스킬
...
- **물고기 44종** — 낚시터별 어종 + **낚일 확률(%)** + 등급 + 인게임 설명
- **필터 / 검색** — 낚시터 칩으로 위치별 보기, 어종명(한/영) 검색

> 데이터는 위키 중앙 데이터 모듈에서 재생성: [`scrape-weapons.mjs`](scrape-weapons.mjs)(무기) · [`scrape-gear.mjs`](scrape-gear.mjs)(방어구/장신구/아이템) · [`scrape-fish.mjs`](scrape-fish.mjs)(낚시).
...
public/
  ...
  weapon-icons/ · gear-icons/ · fish-icons/   # 각 도감 아이콘 + manifest.json
  data/               # weapons.json · armor.json · accessories.json · tools.json · fish.json
```
